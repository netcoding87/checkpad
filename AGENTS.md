...
This Dockerfile is compatible with [Coolify](https://coolify.io/) for self-hosted deployments.

**PostgreSQL Resource Setup:**

1. In Coolify, create or link a PostgreSQL resource to your application
2. Use the resource type "PostgreSQL" and configure:
   - Initial database name: `checkpad` (or your chosen name)
   - Username: Auto-generated by Coolify
   - Password: Auto-generated by Coolify
3. Coolify automatically:
   - Creates the database and user
   - Generates `DATABASE_URL` environment variable
   - Injects it into the deployed container
   - Manages backups and updates
4. Your application connects automatically via the `DATABASE_URL` env var
...
- Add to Coolify startup script: `npm run db:push` (for dev-style push)
- Or: `npm run db:migrate` (for production-style migrations)
